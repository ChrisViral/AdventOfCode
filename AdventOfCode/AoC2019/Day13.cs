...
public sealed class Day13 : IntcodeSolver
{
    ...
    public override void Run()
    {
        ...
        {
            maxX = Math.Max(maxX, (int)this.VM.Output.GetValue());
            maxY = Math.Max(maxY, (int)this.VM.Output.GetValue());

            // We're only interested in the blocks
            if (this.VM.Output.GetValue() == (long)ArcadeObject.BLOCK)
            {
                blocks++;
            }
        }
        ...
        do
        {
            ...
            {
                // Get position
                int x = (int)this.VM.Output.GetValue();
                int y = (int)this.VM.Output.GetValue();
                Vector2<int> position = (x, y);

                // Check if we're receiving the score
                if (position == ScorePos)
                {
                    score = (int)this.VM.Output.GetValue();
                    continue;
                }

                // Set position
                ArcadeObject currentObject = (ArcadeObject)this.VM.Output.GetValue();
            ...
            // Move towards ball
            this.VM.Input.AddValue(ballPos.X.CompareTo(paddlePos.X));
